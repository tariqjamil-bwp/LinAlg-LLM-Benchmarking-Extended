"""
prompts/matrix_vector.py — Build-judge and validate-judge prompts for the MATRIX_VECTOR subcategory.
...
GENERATION_TRUNCATION
  ...
  DETECT: Response ends at "= " or mid-word. No complete vector result present.
...
FORMATTING_MISMATCH
  ...
  incorrectly (wrong vector notation, missing required structure).
  DETECT: Every dot product term and every entry matches ground truth.
  Only the final vector presentation fails the formatting requirement.
...
SIGN_ERROR
  ...
  Product_Sign_Error
    ...
    Vec example: A_{ij}·x_j = (−3)·(4) should give −12 but model writes
    +12 — magnitude correct, sign flipped.

  Operation_Direction
    Model added a term where it should have subtracted, or vice versa,
    when accumulating the dot product sum for an output entry.
    DETECT: Accumulation operator +/− applied in wrong direction.
    Vec example: Running sum should subtract a term but model adds it,
    making the partial sum wrong when it should reduce.

  Rule_Interference
    A negative entry in A or x causes the model to mishandle the sign —
    ...
    Vec example: Entry a = −k; model computes (−k)·x as −(k·x), effectively
    treating the entry as +k.
  ...
  Double_Negative_Trap
    ...
    Vec example: A_{ij} = −3, x_j = −4; model computes (−3)·(−4) = −12
    instead of +12.
...
ARITHMETIC
  ...
  Vec example: Model computes 15 + 0 − 5 − 16 + 15 = −1 instead of 9.
...
HALLUCINATION
  ...
  Complete_Collapse
    ...
    than 2 entries of Ax have been computed. Short response.
  ...
  Teleological_Zeroing
    ...
    despite the terms not summing to zero. Zero asserted, not derived.
    DETECT: Dot product terms shown → sum ≠ 0 → model writes 0 anyway.

  Premature_Assertion
    Model computes some entries of Ax correctly then states the full result
    vector as if complete without computing remaining entries.
    DETECT: Correct partial entries, then sudden full vector claim.

  Silent_Omission
    Model produces a full-length response but silently skips dot product
    computations for one or more entries.
  ...
  Ungrounded_Guess
    Essentially no working shown. Model outputs Ax with minimal or no
    supporting dot product computation.
    DETECT: Very short response, no A_{ij}·x_j terms shown, output vector
    presented as if obvious.

  Dimension_Assertion
    Model returns a matrix-shaped result instead of a 5×1 vector, despite
    some dot product computation being present. The output shape constraint
    is ignored even though working is shown.
    DETECT: Some computation present (at least one dot product shown) but
    the final output is a matrix (e.g. 5×5 or 5×n) instead of a 5×1 vector.
    Disambiguate from Ungrounded_Guess: Dimension_Assertion requires
    computation to be present. Pure assertion with wrong shape = Ungrounded_Guess.
...
───────────────────────────────────────────────────────────────────────────
INPUT_TRANSCRIPTION
  The model miscopied an entry from the ORIGINAL INPUT matrix A or vector x.
  ...
  corresponding entry in the given A or x. Method is correct, arithmetic
...
CARRY_DOWN_ERROR
  ...
  DETECT: Entry (Ax)_i or partial sum stated correctly at step N. Same
...
METHOD_FAIL
  ...
  DETECT: First 3–4 steps do not follow any valid matrix-vector mult method.
  CRITICAL: Only use METHOD_FAIL if the algorithm itself is incorrect.

  (base — no subtype)
    Model never performs row·vector dot products. Examples: computing A·Aᵀ
    instead of Ax, treating x as a row vector throughout (computing xᵀA
    instead of Ax), applying row reduction instead of dot products.
    DETECT: No valid row-of-A dotted-with-x dot products; wrong procedure.

  Operand_Confusion
    Model computes valid dot products but on the wrong arrangement:
    xᵀA instead of Ax (treating x as a row vector dotted with columns of A).
    Dot products structurally correct — only the row/column pairing is wrong.
    DETECT: Valid dot product structure but result is xᵀA not Ax.

───────────────────────────────────────────────────────────────────────────
MEMORY_LOSS
  The model correctly computed and stated an output entry at step N, then
  recalled a different wrong value for that same entry at step M > N.
  DETECT: Entry (Ax)_i stated correctly at one point; wrong value used for
  same entry later without intervening computation.
...
benchmarking LLMs on matrix-vector multiplication computation.

You will be given a 5×5 matrix A, a 5×1 vector x, and a model's FAILED
attempt to compute the product Ax (a 5×1 output vector).

The model may have used any of these strategies:

  STRATEGY 1 — STANDARD DOT PRODUCT
    The model computes each entry (Ax)_i as the dot product of row i of A
    with vector x: sum over j of A_{ij} * x_j.
    Output is a 5×1 column vector with one entry per row.
    Failure mode: arithmetic or sign errors in individual dot product terms
    corrupt one or more entries of the output vector.

  STRATEGY 2 — HEURISTIC / VISUAL INSPECTION
    The model guesses output entries from surface features without computing
...
STEP 1 — COMPUTE THE CORRECT PRODUCT YOURSELF
  You are a 100% accurate matrix-vector multiplication calculator. Compute Ax
  independently by taking the dot product of each row of A with x.
  The output must be a 5×1 column vector. Do not reveal your computation.

STEP 2 — IDENTIFY THE STRATEGY THE MODEL USED
  Based on the model's response, decide which strategy it attempted.
  Record this as "solution_strategy" in your output.

STEP 3 — SCAN THE MODEL RESPONSE FOR THE FIRST WRONG VALUE
  Read the model response step by step from beginning to end.
  Compare every dot product term and every output entry against your correct
  computation. Find the FIRST step where the model's value diverges.

  CRITICAL RULE: Classify by first point of failure, not final symptom.
  If the model makes a sign error in a dot product term for entry 1 and this
  propagates, the correct label is SIGN_ERROR, not ARITHMETIC.
...
For SIGN_ERROR:
{
  ...
  "first_error_step": "Entry 3 (row 3) — dot product term A_{33}·x_3",
  "first_error_description": "Model computed (1)·(−5) = +5 instead of −5. |5|=|−5| confirms sign error.",
  "proposed_novel_tag": "",
  "maps_closest_to": "",
  "confidence": "HIGH"
}

For HALLUCINATION/Dimension_Assertion:
{
  "error_tag": "HALLUCINATION",
  "sign_subtype": "",
  "hallucination_subtype": "Dimension_Assertion",
  "solution_strategy": "DOT_PRODUCT",
  "first_error_step": "Final output assembly",
  "first_error_description": "Model correctly computed entries 1 and 2 of Ax but assembled results into a 5×5 matrix instead of a 5×1 column vector.",
  "proposed_novel_tag": "",
  "maps_closest_to": "",
  "confidence": "HIGH"
}
...
For METHOD_FAIL/Operand_Confusion:
{
  ...
  "sign_subtype": "",
  "hallucination_subtype": "",
  "method_subtype": "Operand_Confusion",
  "solution_strategy": "REPEATED_MULTIPLICATION",
  "first_error_step": "Step 1 — applying power rule",
  "first_error_description": "Valid dot product structure but result is xᵀA not Ax..",
...
For all other tags:
{
  ...
  "hallucination_subtype": "",
  "solution_strategy": "DOT_PRODUCT",
  "first_error_step": "Entry 3 (row 3) — dot product sum",
  "first_error_description": "Model computed 15+0−5−16+15 = −1 instead of 9. Sign correct, |−1|≠|9| confirms arithmetic error.",
  "proposed_novel_tag": "",
  "maps_closest_to": "",
  "confidence": "HIGH"
}
...
Correct Ax: {ground_truth}
Model's extracted Ax answer: {extracted_answer}
...
You will be given:
  1. A square matrix A and a corresponding vector x
  2. A model's failed response attempting to compute Ax
...
YOUR PROCESS:
  Step 1: Compute the correct Ax yourself — dot product of each row of A
          with x. Output must be a 5×1 vector. You are the ground truth oracle.
  Step 2: Read the model response step by step. Compare every dot product term
          and every output entry against your correct computation.
          Find the FIRST step where the model diverges.
...
HALLUCINATION subtypes (required when primary_tag = HALLUCINATION):
  Complete_Collapse     - explicit abandonment phrase, fewer than 2 entries computed
  Teleological_Zeroing  - entry written as 0 despite dot products not summing to 0
  Premature_Assertion   - partial entries computed then full vector claimed complete
  Silent_Omission       - skips dot product blocks silently, no meta-statement
  Ungrounded_Guess      - essentially no dot products shown, just a final vector
  Dimension_Assertion   - computation present but output is a matrix not a 5×1 vector
  Spontaneous_Insertion - fabricated entry inserted after a correct computation chain

METHOD_FAIL subtypes:
  (base)              - wrong algorithm (xᵀA, row reduction, A·Aᵀ, etc.)
  Operand_Confusion   - valid dot products but xᵀA computed instead of Ax
...
RESPOND IN EXACTLY THIS JSON FORMAT AND NOTHING ELSE:
{
  ...
  "proposed_novel_tag":    "",
  "maps_closest_to":       "",
  "forensic_observation":  "Confirmed: model computed entry 3 as 15+0−5−16+15=−1 instead of 9. Signs correct throughout, magnitude wrong.",
  "error_scope":           "isolated",
  "affected_positions":    ["entry 3 (row 3)"],
  "confidence":            "HIGH"
}
...
affected_positions:   list the specific output entries or dot product steps
                      where the error manifests (e.g. ["entry 2", "entry 4"])
""").substitute(
    MAGNITUDE_RULE=MAGNITUDE_RULE,
...
ends mid-expression or mid-computation with no final vector result.

Verification question:
  (1) Confirm the response has no complete Ax vector in the final answer.
  (2) Quote the last ~50 characters of the response. Does it end mid-sentence,
      mid-expression (e.g. "= "), or mid-dot-product?
  (3) Does the response contain any complete Ax claim anywhere?

CRITICAL: If a complete Ax result exists anywhere in the response, this is
...
matrix-vector computation is mathematically correct but the format is wrong.

Verification question:
  (1) Verify every dot product term and every entry of Ax matches your
      correct computation.
  (2) Identify the specific formatting failure (wrong vector notation, etc.).
...
miscopied an entry from the original input matrix A or vector x.

Verification question:
Identify the EXACT entry that was miscopied. State:
  (1) Which source (A or x), row and column (e.g., A_{2,3} or x_4),
  (2) Correct value at that position,
  (3) Value the model actually used in the dot product instead.

SCOPE CHECK (mandatory):
Check ALL adjacent entries in the same row/column of the model's written
submatrix. If the model replaced an entire row or column with fabricated data,
reclassify to HALLUCINATION (Silent_Omission or Spontaneous_Insertion).

CRITICAL: You must find both values directly in the response text.
Exact source, position, and both values required."""

    # ── CARRY_DOWN_ERROR ──────────────────────────────────────────────────
    elif tag == "CARRY_DOWN_ERROR":
        return """The junior judge classified this as CARRY_DOWN_ERROR — the model
correctly stated a dot product term or entry at step N but miscopied it
at step N+1 when carrying it forward.
...
Verification question:
Find the specific output entry where the wrong number first appeared.
  (1) Were ALL signs in that dot product correct?
  (2) Apply the MAGNITUDE RULE: is |wrong value| ≠ |correct value|?
  (3) Was the row·vector pairing correct?
...
the model used the correct magnitude for a dot product term A_{{ij}}·x_j
but applied the wrong sign to it.
Junior judge's first error step: {step or '(not specified)'}

Verification question:
Find the exact dot product term. State:
  (1) Which output entry (row i) and which term j,
  (2) Correct values of A_{{ij}} and x_j and their correct product,
...
If wrong entry values used, reclassify as INPUT_TRANSCRIPTION."""
...
the model added a term where it should have subtracted (or vice versa)
when accumulating the dot product sum.
Junior judge's first error step: {step or '(not specified)'}

Verification question:
Find the exact accumulation step. State:
  (1) Which output entry (row i) and which accumulation step,
...
of the individual term. If term's own sign was flipped, reclassify as
Product_Sign_Error."""
...
a negative entry in A or x triggered wrong sign behaviour during multiplication.
Junior judge's first error step: {step or '(not specified)'}

Verification question:
  (1) Which entry caused the interference? State source (A or x) and position.
...
Verification question:
  ...
  (3) Confirm BOTH factors are genuinely negative (from A and x).
  (4) Apply MAGNITUDE RULE: confirm |product| is correct — only sign wrong.
...
Verification question:
  ...
  (2) Identify the first entry with wrong sign — state row i, correct value,
      and model's value.
...
term and reclassify (Product_Sign_Error, Operation_Direction, Rule_Interference,
or Double_Negative_Trap)."""
...
Populate sign_subtype in your response."""
    ...
    elif tag == "HALLUCINATION" and halluc_sub == "Dimension_Assertion":
        return """The junior judge classified this as HALLUCINATION/Dimension_Assertion —
the model returned a matrix instead of a 5×1 vector despite showing some
dot product computation.

Verification question:
  (1) What shape is the model's output? (e.g. 5×5, 5×2, etc.)
  (2) Is any dot product computation shown before the wrong-shape output?
      Quote at least one dot product term from the response.
  (3) Does the model acknowledge or show awareness that Ax should be a vector?

CRITICAL: Dimension_Assertion requires SOME computation to be present.
If no computation shown and model just outputs a matrix, reclassify as
Ungrounded_Guess."""
...
Verification question:
  (1) Which output entry (row i) was written as 0?
...
CRITICAL: If the shown terms DO correctly sum to 0, this is a correct entry.
The defining feature is arithmetic that does NOT support zero."""
...
than 2 output entries were computed.

Verification question:
  (1) Quote the exact phrase where the model abandoned computation.
  (2) How many output entries did the model actually compute? Must be fewer than 2.
  (3) Did the model show any valid dot products at all?

CRITICAL: If 2 or more entries computed before stopping, reclassify as
Premature_Assertion."""
...
the model computed some entries correctly then stated the full vector as complete.

Verification question:
  (1) Which output entries (rows) did the model compute with dot products shown?
  (2) Which entries are absent when the full vector is claimed?
  (3) Quote the line where the full vector is asserted prematurely.

CRITICAL: At least one valid entry must have been computed. If none, use
...
Verification question:
  ...
  (3) Does the response jump from partial computation to the full vector?
...
essentially no dot product working shown; model states Ax with no derivation.

Verification question:
  (1) Are there any valid dot product terms shown for any output entry?
...
If computation IS present but output has wrong shape, reclassify as
Dimension_Assertion."""
...
Verification question:
  (1) Identify the last entry correctly derived — state row i and value.
  (2) Identify the inserted entry that has no prior basis — state row i
...
Determine which subtype applies:
  Complete_Collapse     — explicit abandonment + fewer than 2 entries computed
  Teleological_Zeroing  — entry written as 0 despite dot products not summing to 0
  Premature_Assertion   — partial entries computed then full vector claimed
...
a correct output entry earlier and stated it, then used a different wrong
value for that same entry in a later step (recalled after ≥1 unrelated steps).

Verification question:
  (1) Which entry (row i) was stated correctly at the early step?
...
performed legitimate matrix-vector multiplication (row·vector dot products).

Verification question:
Describe what method the model used in the first 3–4 steps.

If the model computed xᵀA instead of Ax (treating x as a row vector),
state this explicitly — this is Operand_Confusion.

Examples that qualify as base METHOD_FAIL:
  - Computing A·Aᵀ or A·A instead of Ax
  - Applying row reduction to find Ax
  - Treating Ax as a scalar computation

Examples that qualify as Operand_Confusion:
  - Valid dot products but x treated as row vector: result is xᵀA not Ax
  - Column of A dotted with rows of x

CRITICAL: If the model set up legitimate dot products but made errors inside
them, classify by actual error type and set verified=false."""
...
Read the model's matrix-vector multiplication from the beginning.
Find the first step where the model's value diverges from correct Ax.
...
For HALLUCINATION, populate hallucination_subtype:
  ...
  Silent_Omission       | Ungrounded_Guess     | Dimension_Assertion |
  Spontaneous_Insertion
...
def build_validate_user_prompt(row: dict, response: str, matrix_latex: str) -> str:
    ...
    return f"""Matrix A and vector x:
{matrix_latex}
